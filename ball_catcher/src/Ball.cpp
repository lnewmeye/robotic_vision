...
int Ball::captureBall(Mat left_image, Mat right_image)
{
	// Set background if not set initialize object
	if (left_background.empty() || right_background.empty()) {
		initialize(left_image, right_image);
		return 0;
	}

	// Create variables for switch statement
	bool detected;

	// Current state logic
	switch(catch_state) {
		case CATCH_DETECT:
			detected = detectBall(left_image, right_image);
			if(detected)
				ball_count++;
			break;
	...
	switch(catch_state) {
		case CATCH_DETECT:
			if (detected) {
				frame_count = 0;
				catch_state = CATCH_TRACK;
			}
			break;

		case CATCH_TRACK:
			if (frame_count > BALL_DETECTIONS_MAX) {
				ball_count = 0;
				catch_state = CATCH_WAIT;
			}
			break;

		case CATCH_WAIT:
			if (frame_count > FRAME_WAIT_MAX) {
				frame_count = 0;
				reset(left_image, right_image);
				catch_state = CATCH_DETECT;
			}
			break;
	}
...
void Ball::reset(Mat left_image, Mat right_image)
{
...
void Ball::initialize(Mat left_background, Mat right_background)
{
	...
	left_intrinsic = (cv::Mat_<double>(3,3) << CALIBRATION_LEFT_INTRINSIC);
	left_distortion = (cv::Mat_<double>(5,1) << CALIBRATION_LEFT_DISTORTION);
	right_intrinsic = (cv::Mat_<double>(3,3) << CALIBRATION_RIGHT_INTRINSIC);
	right_distortion = (cv::Mat_<double>(5,1) << CALIBRATION_RIGHT_DISTORTION);
	rotation = (cv::Mat_<double>(3,3) << CALIBRATION_STEREO_ROTATION);
	translation = (cv::Mat_<double>(3,1) << CALIBRATION_STEREO_TRANSLATION);
	essential = (cv::Mat_<double>(3,3) << CALIBRATION_STEREO_ESSENTIAL);
	fundamental = (cv::Mat_<double>(3,3) << CALIBRATION_STEREO_FUNDAMENTAL);
...
bool Ball::detectBall(Mat left_image, Mat right_image)
{
	...
	if (detected) {
		...
		// Push ball points to vector
		Point2f point;
		point.x = left_ball.centroid_x;
		point.y = left_ball.centroid_y;
		left_points.push_back(point);
		point.x = right_ball.centroid_x;
		point.y = right_ball.centroid_y;
...
void Ball::predictTrajectory(double& x_predict, double& y_predict)
{
	// Rectify left and right and find 3d points
	vector<Point3f> points3d = transformPoints();

	// Form points into x, y and z vectors
	vector<float> x, y, z;
	...
	// Solve least squares solution for points
	//Mat coef_x = solveLsqQuadratic(z, x);
	Mat coef_x = solveLsqLinear(z, x);
	Mat coef_y = solveLsqQuadratic(z, y);

	//x_predict = (double)coef_x.at<float>(0, 2);
	x_predict = (double)coef_x.at<float>(0, 1);
	y_predict = (double)coef_y.at<float>(0, 2);
}

vector<Point3f> Ball::transformPoints()
{
	// Create vectors for undistorted points
	vector<Point2f> left_undistorted, right_undistorted;

	// Undistort left and right points
	cv::undistortPoints(left_points, left_undistorted, left_intrinsic, 
			left_distortion, left_rectification, left_projection);
	cv::undistortPoints(right_points, right_undistorted, right_intrinsic, 
			right_distortion, right_rectification, right_projection);
	...
	for (unsigned i = 0; i < left_undistorted.size(); i++) {
		point3d.x = left_undistorted[i].x;
		point3d.y = left_undistorted[i].y;
		point3d.z = left_undistorted[i].x - right_undistorted[i].x;
	...
	// Adjust points to catcher coordinates
	vector<Point3f> catcher_points;
	for (Point3f point : points_transformed) {
		point.x = BALL_CATCHER_X - point.x;
		point.y = BALL_CATCHER_Y - point.y;
		point.z = point.z - BALL_CATCHER_Z;
		catcher_points.push_back(point);
	}
	
	return catcher_points;
}

Mat Ball::solveLsqQuadratic(vector<float> Z, vector<float> B)
{
	// Create matrix structures
	Mat A(Z.size(), 3, CV_32FC1);
	Mat b(B.size(), 1, CV_32FC1);
	Mat x;

	// Form matricies A and b
	int index = 0;
	for (float z : Z) {

		// Set quatratic values
		A.at<float>(index,0) = z*z;
		A.at<float>(index,1) = z;
		A.at<float>(index,2) = 1;
		b.at<float>(index,0) = B[index];
...
Mat Ball::solveLsqLinear(vector<float> Z, vector<float> B)
{
	// Create matrix structures
	Mat A(Z.size(), 2, CV_32FC1);
	Mat b(Z.size(), 1, CV_32FC1);
	Mat x;
	...
	for (float z : Z) {
		...
		// Set quatratic values
		A.at<float>(index,0) = z;
		A.at<float>(index,1) = 1;
		b.at<float>(index,0) = B[index];
